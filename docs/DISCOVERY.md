# Discovery Metadata

Use this file as the source of truth for public discovery copy across GitHub About, repository topics, README, `skill.json`, and `.codex-plugin/plugin.json`.

## GitHub About

Description:

```text
ATO-backed Australian tax prep skills for Codex, Claude Code, and Cowork: GST/BAS, CGT, evidence gaps, and accountant handoff.
```

Website:

```text
https://github.com/nijanthan-dev/taxmate-australia#readme
```

Topics:

```text
abn
accountant
ato
australian-tax
bas
cgt
claude-code
codex
codex-plugin
cowork
fbt
gst
openagentskill
payg
sole-trader
superannuation
tax-prep
tax-records
```

## Keyword Map

- Primary: Australian tax prep, ATO-backed tax prep, tax records, accountant handoff.
- Topic clusters: GST/BAS, CGT, PAYG, FBT, ABN, sole trader, superannuation, private health, crypto, rental property.
- Agent/platform terms: Codex plugin, Claude Code skills, Cowork skills, OpenAgentSkill CLI.

## Copy Rules

- Lead with outcomes, not implementation.
- Keep prep-only/no-advice/no-lodgment boundaries visible.
- Do not imply ATO, government, insurer, super fund, or financial-institution endorsement.
- Do not claim marketplace approval or official listing until verified.
- Do not reintroduce Go runtime wording.
