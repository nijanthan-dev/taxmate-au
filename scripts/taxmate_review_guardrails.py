...
REVIEW_PATTERNS: List[ReviewPattern] = [
    ...
    ReviewPattern(
        ...
        INDIVIDUAL_INTAKE_CONTRACT,
        "Investment income intake must keep missing statements, common not-provided statement wording, absent core amount/component values, unknown or malformed amounts, uncertain franking, AMIT/cost-base adjustments, foreign components, trust distributions, scalar flat investment fields, relevant aggregate-vs-item total conflicts, and supplied aggregates with any unknown relevant item total in Evidence or Accountant review while preserving aggregate-only base rows, zero withholding, zero franking credits, false foreign components, zero aggregate reconciliation for empty investment categories, source provenance, flat/nested item aliases, and direct dividend/distribution amount aliases.",
    ),
    ReviewPattern(
        "Issue #51 PSI",
        INDIVIDUAL_INTAKE_CONTRACT,
